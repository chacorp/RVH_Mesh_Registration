...
def render_dispmap(args):
    ...
    ### UV coordiante
    _, faces, aux    = load_obj('./assets/temp.obj', load_textures=False)
    ...
    pil_mask     = Image.open('./assets/mask_soft.png')
    ...
    #     with open( f"./results/{basename}",'w') as final_file:
    ...
    img[3][img[3] > 0] = 1.0
    # tv.utils.save_image(img, f"results/{basename}_crop_displacement_map_raw.png")
...
#     (1 - np_mask) * 0.5 
    ...
    Image.fromarray(np_img_refine).save(f"results/{basename}_crop_normal_map.png")
    print('>>> Done.')
